...
const dataPutJSON = require('../../fixtures/putuser.json')
...
// PUT user (PUT) WE update information in the body above.
                ...
                cy.request({
                    ...
                    body : {
                        "name": dataPutJSON.name,
                        "email": dataPutJSON.email,
                        "gender": dataPutJSON.gender,
                        "status": dataPutJSON.status
                }

                }).then((res)=>{
                    expect(res.status).to.eq(200)
                    expect(res.body).has.property('id', parseInt(userId))
                    expect(res.body).has.property('name', dataPutJSON.name)
                    expect(res.body).has.property('email', dataPutJSON.email)
                    expect(res.body).has.property('status', dataPutJSON.status)
